...
export const getAdminDashboard = async (req, res) => {
  try {
    ...
    const [
      ...
      User.countDocuments({ role: { $not: { $regex: "^admin$", $options: "i" } } }),
      Order.find({ createdAt: { $gte: start, $lte: end } })
        .populate("user", "name email")
        .populate("products.product", "name image")
        .sort({ createdAt: -1 }),
      Order.find({ createdAt: { $gte: previousStart, $lt: previousEnd } }).select(
    ...
    const conversionVisitors = await Visitor.countDocuments({
    ...
    const conversionRate = currentVisitors === 0
      ? null
      : round((conversionVisitors / currentVisitors) * 100);

    const previousConversionVisitors = await Visitor.countDocuments({
      createdAt: { $gte: previousStart, $lt: previousEnd },
      converted: true,
    });
    ...
    const previousConversionRate = previousVisitors === 0
      ? null
      : (previousConversionVisitors / previousVisitors) * 100;
    ...
    const categoryData = categoryBreakdown.map((item) => ({
      name: item.name,
      count: item.count,
      percent: categoryTotal === 0 ? 0 : round((item.count / categoryTotal) * 100),
    }));

    const lowInventory = await Variant.aggregate([
      { $match: { stock: { $lte: 15 } } },
      {
        $lookup: {
          from: "products",
    ...
    const chartRows = await Order.aggregate([
      { $match: { createdAt: { $gte: chartStart, $lte: chartEnd }, paymentStatus: "paid", orderStatus: { $ne: "cancelled" } } },
      { $group: { _id: chartGroup, revenue: { $sum: "$totalPrice" } } },
      { $sort: { _id: 1 } },
    ]);
    ...
    const dashboard = {
      ...
      range: { start, end },
      stats: {
        totalRevenue: round(Number(allTimeRevenue[0]?.total ?? 0)),
        periodRevenue: round(currentRevenue),
        ...
        conversionRate,
        lowStockAlerts: lowInventory.length,
        changes: {
          revenue: percentageChange(currentRevenue, previousRevenue),
          orders: percentageChange(currentOrders.length, previousOrders.length),
          visitors: percentageChange(currentVisitors, previousVisitors),
          conversionRate: percentageChange(conversionRate ?? 0, previousConversionRate ?? 0),
        },
      },
      categoryBreakdown: categoryData,
      revenueChart: chartRows.map((row) => ({
        period: row._id,
        label: period === "year"
          ? new Date(2000, Number(row._id) - 1, 1).toLocaleString("en-US", { month: "short" })
          : period === "week"
            ? ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"][Number(row._id) - 1]
            : String(row._id),
...
export const trackVisitor = async (req, res) => {
  try {
    const { visitorId, path = null } = req.body;
    ...
    const visitor = await Visitor.findOneAndUpdate(
      { visitorId, monthKey },
      {
        $set: { lastSeenAt: now, ...(path ? { lastPath: path } : {}) },
        $setOnInsert: {
          visitorId,
          monthKey,
          createdAt: now,
