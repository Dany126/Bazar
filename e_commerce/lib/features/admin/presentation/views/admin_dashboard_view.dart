...
class AdminDashboardView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return MultiBlocProvider(
      providers: [
        BlocProvider(create: (_) => getIt<AdminDashboardCubit>()..loadDashboard()),
...
class _AdminDashboardBody extends StatelessWidget {
  ...
  Widget _content(int index, bool wide, bool medium) {
    switch (index) {
      case 0: return _dashboard(wide, medium);
      case 1: return const AdminOverviewView();
      case 2: return const AdminCustomersView();
      case 3: return const AdminProductsView();
      case 4: return const AdminWorkspaceView();
      case 5: return const AdminSettingsView();
      case 6: return const AdminEarningsView();
      case 7: return const AdminPayoutsView();
      default: return const Center(child: Text('No data'));
    }
  }
    ...
    return BlocBuilder<AdminDashboardCubit, AdminDashboardState>(
      builder: (context, state) {
        ...
        return RefreshIndicator(
          onRefresh: () => context.read<AdminDashboardCubit>().loadDashboard(period: data.period),
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Row(children: [
                ...
                DropdownButton<String>(
                  ...
                  onChanged: (value) { if (value != null) context.read<AdminDashboardCubit>().loadDashboard(period: value); },
                ),
              ]),
              ...
              if (wide) Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Expanded(flex: 2, child: AdminDashboardSalesOverview(points: data.revenueChart, period: data.period)),
                const SizedBox(width: 20),
                Expanded(child: AdminDashboardCategories(categories: data.categoryBreakdown)),
              ]) else Column(children: [
                AdminDashboardSalesOverview(points: data.revenueChart, period: data.period),
                const SizedBox(height: 20),
                AdminDashboardCategories(categories: data.categoryBreakdown),
              ]),
              const SizedBox(height: 24),
              if (medium) Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Expanded(flex: 2, child: AdminDashboardRecentOrders(orders: data.recentOrders)),
                const SizedBox(width: 20),
                Expanded(child: AdminDashboardInventory(items: data.lowInventory)),
              ]) else Column(children: [
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, constraints) {
      ...
      return BlocBuilder<AdminNavigationCubit, int>(builder: (context, index) {
        final sidebar = AdminSidebar(selectedIndex: index, onItemSelected: (i) {
        ...
        return Scaffold(
          ...
          body: SafeArea(child: Row(children: [if (sidebarVisible) sidebar, Expanded(child: _content(index, wide, medium))])),
        );
      });
    });
  }
